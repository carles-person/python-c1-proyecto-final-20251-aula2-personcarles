...
class Appointment(db.Model):
    ...
    def to_dict(self):
        return {
            ...
            'time': self.dt_start.time().isoformat(),
    ...
    def from_json(self,json_data)->bool:
        try:
            ...
            temp_start = dt.time.fromisoformat(json_data.get('time'))
            temp_start = dt.time(temp_start.hour, int(temp_start.minute /15)*15,0) # arrodoniment a 15minuts
            temp_duration = int(int(json_data.get('duration'))/15)*15 #arrodoniment a 15min
