...
def add_appointment():
    ...
    try:
        return requests.put(f'{current_app.config['OC_AGENDA_SVC']}/add', headers=header, data= data)
    except:
...
def get_appointments():
    ...
    if date_start !='':
        query_str += f'date_start={date_start}&'
    if date_end !='':
        query_str += f'date_end={date_end}&'
    
    query_str += f'max_results={max_results}'
