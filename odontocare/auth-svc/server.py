...
def create_app():
    ...
    app.config['AUTH_LISTEN'] = os.environ.get('AUTH_LISTEN', '0.0.0.0')
    app.config['AUTH_PORT'] = os.environ.get('AUTH_PORT', 4001)
    ...
    def login():
        ...
        return json_error(ERR_CREDENTIALS)
    ...
    def validate(user_id):
        ...
        if not token:
            return json_error(ERR_MISSING_TOKEN)
    ...
    def register():
        ...
        try:
            ...
            return json_error(ERR_REGISTRATION)

        return jsonify({'id': new_user.id, 'username': new_user.username, 'role': new_user.role})
...
if __name__ == '__main__':
    app = create_app()
    app.run(host=app.config['AUTH_LISTEN'], port = app.config['AUTH_PORT'])